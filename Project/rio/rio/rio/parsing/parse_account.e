		...
		do
			...
			if row_temp.matches_regex ("^\s*(?i)Account\s*#?\s*:?\s*$") then -- keyword name is found
				...
				if row_temp.is_empty_from (row_temp.index_of ("Account") + 1) then
					...
					error.custom_msg ("Account field is empty on line" + row_temp.number.out + ". only Account keyword is found.%N")
				else
						-- contains words
					if row_temp.matches_regex ("\s*\d+\s*") then
						...
						error.custom_msg ("Account field is invalid on line" + row_temp.number.out + ". make sure to have only numbers.%N")
					end
				end
			end
